...
contains
   ...
   subroutine append_msg_line(line, err, &
      ...
      if( present( comment ) ) new%comment = comment
      new%icomment = get_comment_icode( comment )
      if( present( whichsub ) ) new%whichsub = whichsub
      new%iwhichsub = get_subname_icode( whichsub )
      if( present( whichmod ) ) new%whichmod = whichmod
